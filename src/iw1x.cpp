...
void SV_AuthorizeRequest(netadr_t from, int challenge)
{
    ...
    NET_OutOfBandPrint(NS_SERVER, svs.authorizeAddress, va("getIpAuthorize %i %i.%i.%i.%i %s %i", challenge, from.ip[0], from.ip[1], from.ip[2], from.ip[3], gameDir, sv_allowAnonymous->integer));
}
...
void custom_SV_SendClientGameState(client_t* client)
{
    ...
    for (start = 0; start < MAX_GENTITIES; start++) {
        ...
        ((void (*)(msg_t*, struct entityState_s*, int*, qboolean))0x807f698)(&msg, &nullstate, base, qtrue); // MSG_WriteDeltaEntity
    }
...
template <std::size_t N> bool isValidBanParameter(std::string toCheck, std::array<std::string, N> parameters)
{
...
void custom_SV_ExecuteClientMessage(client_t* cl, msg_t* msg)
{
    ...
    */
    if ((cl->serverId == sv_serverId_value || cl->downloadName[0]) || (!cl->downloadName[0] && strstr(cl->lastClientCommandString, "nextdl"))) {
        do {
...
void SV_CalculatePing(client_t* cl)
{
    ...
    {
        int idx = (current - i) & PACKET_MASK;

        int sent = cl->frames[idx].messageSent;
        int ack  = cl->frames[idx].messageAcked;
...
void CrashLogger(int sig)
{
    ...
    if (len == -1) {
        ...
        if (system("stty sane") != 0) {
            fprintf(stderr, "Failed to execute 'stty sane' in CrashLogger\n");
        }
        exit(1);
    }
    lnxded_path[len] = '\0';
...
class iw1x
{
  public:
    iw1x()
    {
        ...
        // Clang __VERSION__ expands to: 'Clang x.x.x' or 'Debian Clang x.x.x'
        printf("Compiler:     %s\n", __VERSION__);
    ...
    ~iw1x()
    {
        ...
        if (system("stty sane") != 0) {
            fprintf(stderr, "Failed to execute 'stty sane' in destructor\n");
        }
    }
};
